...
from temp import probit
import data

from sklearn.linear_model import ARDRegression, LinearRegression, lasso_path
...
def write_predictions(P, prefix, rsids, gt, las_predict, fsr_predict, ard_predict, 
    ...
    results = np.vstack([rsidsx, gt, las_predict, fsr_predict, ard_predict, bgs0_predict, bgs1_predict, bgs2_predict, map0, map1, map2]).T
...
def test_files(path, prefix, P, burnin, iters):
    ...
    for fn in onlyfiles:
        if prefix in fn and 'yx_' in fn and '_'+str(P)+'.' in fn:
            ...
            model0 = probit.ProbitSS(X, y, data.bend_corr(corr0))
            ...
            model2 = probit.ProbitSS(X, y, data.bend_corr(corr2))
...
def run_test(path, prefix, dims, burnin, iters):
    ...
    model0 = probit.ProbitSS(X, y, np.eye(corr.shape[0]))
    inclusion_probs0, map0 = model0.run_mcmc(burnin=burnin, iters=iters)  
    ...
    model2 = probit.ProbitSS(X, y, cov)
    inclusion_probs2, map2 = model2.run_mcmc(burnin=burnin, iters=iters)  
    print("Covariance S-S complete")
