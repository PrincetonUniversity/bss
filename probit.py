...
class ProbitSS:
    ...
    def log_marg_like(self, gamma, gamma0, nu, lamb):
        ...
        lml = -(0.5*self.num_data*np.log(2*np.pi) + np.sum(np.log(np.diag(chol_cov))) + 0.5*np.dot(solve.T, solve))
        return lml
    ...
    def log_gamma_prior(self, gamma):
        ...
        return -(0.5*self.num_snps*np.log(2*np.pi) + np.sum(np.log(np.diag(self.cR))) + 0.5*np.dot(solve.T, solve))
...
if __name__ == '__main__':
    ...
    #         model.lamb,
    #         1/model.nu,
    #         np.sum(model.gamma > model.gamma0),
    #         np.sum(eqtls == (model.gamma > model.gamma0))
    #    )
    #    model.update_gamma()
    #    model.update_gamma0()
    #    model.update_lambda()
