...
from bss.utils.math import *
from bss.utils.mcmc import elliptical_slice, slice_sample, uni_slice_sample
...
class ProbitSS(object):
    ...
    def log_marg_like(self, gamma, gamma0, nu, lamb):
        ...
        try:
            ...
            solve = spla.solve_triangular(chol_cov / np.sqrt(nu), self.Y, lower=True, trans=0,
                                          check_finite=self.check_finite)

            self._log_marg_like = -0.5 * self.N * np.log(2 * np.pi / nu) - np.sum(
                np.log(np.diag(chol_cov))) - 0.5 * np.dot(solve.T, solve)
            self._last_chol_cov = chol_cov
    ...
    def update_xi(self):
        ...
        if True:
            ...
            whitened = spla.solve_triangular(self.get_cholesky(), self.gamma, lower=True, trans=0,
                                             check_finite=self.check_finite)
            ...
            def slice_fn(xi):
                ...
                try:
                    ...
                    solve = spla.solve_triangular(chol_cov, self.gamma, lower=True, trans=0,
                                                  check_finite=self.check_finite)
                    return (-0.5 * self.P * np.log(2 * np.pi) - np.sum(np.log(np.diag(chol_cov))) - 0.5 * np.dot(
                        solve.T, solve)
    ...
    def log_gamma_prior(self, gamma):
        ...
        return -0.5 * self.P * np.log(2 * np.pi) - np.sum(np.log(np.diag(self.get_cholesky()))) - 0.5 * np.dot(solve.T,
                                                                                                               solve)
